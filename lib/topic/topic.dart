...
class AddTopicForm extends StatefulWidget {
  @override
  _AddTopicFormState createState() => _AddTopicFormState();
}
...
class _AddTopicFormState extends State<AddTopicForm> {
    ...
    try {
      ...
      multiSelectController.addItems(snapshot.docs.map((doc) {
        return DropdownItem<String>(
          value: doc.id,
          label: doc['subjectName'], // Assuming each category has a 'name' field
        );
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      final subject = {
        'topicName': _subjectNameController.text.trim(),
        'subjectId': _selectedCourses.first,
      };

      try {
        var doc = await FirebaseFirestore.instance.collection('topics').add(subject);
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              Text('Select Subjects:', style: TextStyle(fontWeight: FontWeight.bold)),
